...
def getStrUni(channel,num_extractions):
    channel = channel[0,:]
    max_ch = np.max(channel)
    min_ch = np.min(channel)
    extracted_gains = np.random.uniform(min_ch,max_ch,np.int32(num_extractions))
    strategy_estim = np.zeros(ch.shape)[0,:]
    for el in extracted_gains:
        difference = np.abs(channel-el)
        index =difference.argmin()
        strategy_estim[index] = strategy_estim[index]+1
    strategy_estim=strategy_estim/(np.sum(strategy_estim))
    return strategy_estim
