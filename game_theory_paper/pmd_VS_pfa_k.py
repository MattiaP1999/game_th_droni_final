...
#p_fas = [1e-2]
n_pfas = 20
p_fas = np.logspace(-4,-1,n_pfas, True, 10)
sigmas = [3,10,16]
...
for K in k:
    ...
    for pfa in p_fas:
        ...
        for kk in ch_real:
            filename = './data_from_python/complete_dataset/data_gth_python_10_3_'+str(kk+100)+'.mat'
        ...
        results_valgame[index_K,pfa_index]=val_games.mean()
        results_valgame_checks[index_K,pfa_index]=val_games_check.mean()
        results_uniforme[index_K,pfa_index] = val_games_uniforme.mean()
        counter = counter +1
        print(str(counter/np.float16(results_valgame.size)*100))
        pfa_index=pfa_index+1
    index_K=index_K+1

sp.io.savemat("Results_pmd_pfa_k.mat", dict(values_game=results_valgame,values_game_uniforme = results_uniforme, results_valgame_sim = results_valgame_checks, p_fa = p_fas))
""""
    # plotting
fig, ax = plt.subplots()


ax.semilogy(p_fas, results_valgame[0,:], label="K=3")
ax.semilogy(p_fas, results_valgame[1,:], label="K=10")
ax.semilogy(p_fas, results_valgame[2,:], label="K=16")
ax.semilogy(p_fas, results_uniforme[0,:], label="K=3, uniforme")
ax.semilogy(p_fas, results_uniforme[1,:], label="K=10, uniforme")
ax.semilogy(p_fas, results_uniforme[2,:], label="K=16,uniforme")
